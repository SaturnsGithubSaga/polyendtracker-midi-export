...
class MIDITrack(object):
    ...
    def deInterleaveNotes(self):
       ...
       # FIX: Polyend Tracker patterns often contain "orphan" NoteOff events (e.g. at the start of a pattern to kill sound).
       # The original logic assumed every NoteOff had a preceding NoteOn, causing an "IndexError: pop from empty list".
       # We added a check to ensure the note exists in the stack before popping. Orphan NoteOffs are now preserved safely without crashing.
                elif event.evtname == 'NoteOff':
                    # FIX: Handle orphan NoteOff events (e.g. at start of pattern) to prevent crashes.
                    # We check if the note exists in the stack AND if the stack is not empty.
                    if noteeventkey in stack and stack[noteeventkey]:
                    ...
                    # If no matching NoteOn is found (orphan NoteOff), just add the event as-is without popping.
                        tempEventList.append(event)
           # Old code:
           #     elif event.evtname == 'NoteOff':
           #         if len(stack[noteeventkey]) > 1:
